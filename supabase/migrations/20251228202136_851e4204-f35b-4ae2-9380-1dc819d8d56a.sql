-- Fix RLS policies to use Clerk's 'sub' claim instead of 'user_id'
-- Clerk JWTs use 'sub' for the user ID, not 'user_id'
...
FOR SELECT USING (clerk_user_id = ((current_setting('request.jwt.claims'::text, true))::json ->> 'sub'::text));

CREATE POLICY "Users can insert own record" ON public.users
FOR INSERT WITH CHECK (clerk_user_id = ((current_setting('request.jwt.claims'::text, true))::json ->> 'sub'::text));

CREATE POLICY "Users can update own record" ON public.users
FOR UPDATE USING (clerk_user_id = ((current_setting('request.jwt.claims'::text, true))::json ->> 'sub'::text))
...
FOR DELETE USING (clerk_user_id = ((current_setting('request.jwt.claims'::text, true))::json ->> 'sub'::text));
